...
3、应用函数
    MFRC_Init();//初始化
    RC522_Reset();//器件复位
    RC522_Request(PICC_REQALL, RxBuffer);//返回值为0，代表寻卡成功；并把卡类型存入RxBuffer中
    RC522_Anticoll(RxBuffer);   //把（十六进制）的4个字节卡号存储在数组RxBuffer中
***********************************/
...
/**************************************************************************************
* 函数名称：RC522_AntennaOn
* 功能描述：开启天线,使能RC522发送能量载波信号
* 入口参数：无
* 出口参数：无
* 返 回 值：无
...
/**************************************************************************************
* 函数名称：RC522_AntennaOff
* 功能描述：关闭天线,失能RC522发送能量载波信号
* 入口参数：无
* 出口参数：无
* 返 回 值：无
* 说    明：每次开启或关闭天线之间应至少有1ms的间隔
...
char RC522_AuthState(uint8_t AuthMode, uint8_t BlockAddr, uint8_t *pKey, uint8_t *pSnr)
{
    ...
    if((status != MI_OK) || (!(MFRC_ReadReg(MFRC_Status2Reg) & 0x08)))
    {
        status = MI_ERR;
    }
...
char RC522_WriteBlock(uint8_t BlockAddr, uint8_t *pData)
{
    ...
    if((status != MI_OK) || (unLen != 4) || ((CmdFrameBuf[0] & 0x0F) != 0x0A))
    {
        status = MI_ERR;
    }
