...
/*RC522通讯时返回的错误代码*/
#define MI_OK                 	(char)0				//成功
#define MI_NOTAGERR            (char)(-1)			//无卡
#define MI_ERR                	(char)(-2)			//出错
...
char RC522_Request(uint8_t RequestMode, uint8_t *pCardType);  //寻卡，并返回卡的类型
char RC522_Anticoll(uint8_t *pSnr);                           //防冲突,返回卡号
char RC522_Select(uint8_t *pSnr);                             //选卡
char RC522_AuthState(uint8_t AuthMode, uint8_t BlockAddr, uint8_t *pKey, uint8_t *pSnr); //验证密码(密码A和密码B)   
char RC522_WriteBlock(uint8_t BlockAddr, uint8_t *pData);   //写数据
char RC522_ReadBlock(uint8_t BlockAddr, uint8_t *pData);    //读数据
